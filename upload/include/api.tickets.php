...
class TicketApiController extends ApiController {
    ...
    */
    ...
    /**
     * RESTful GET last tickets
     * 
     * $limit the limit of last tickets
     * url dispatcher.
     */
    function restGetLastTickets($offset, $limit) {
        ...
        // To allow routes like api/.../1/5 (meaning starting from position 1 and count 5 items)
        $offset = $offset > 0 ? $offset-1 : $offset;
        $limit = $limit > 0 ? $limit-1 : $limit;

        $range_len = $limit - $offset + 1;
        if ($range_len < 1)
        ...
        $range_high_request = $offset + $range_len - 1;
        header("X-Request-Range: items=${offset}-${range_high_request}");
        # Build query
        $qfields = array('t.ticket_id id', 't.number', 'e.title', 'e.poster', 'u.address', 's.state', 't.created', 't.updated', 't.closed',
             'r.lastmessage', 'r.lastresponse');
        $q = 'SELECT ';
        foreach ($qfields as $f) {
            $q .= $f.',';
        }
        $q = rtrim($q, ',');
        $q .= ' FROM '.TICKET_TABLE.' t ';
        $q .= ' INNER JOIN '.THREAD_TABLE.' r ';
        $q .= ' ON r.object_id = t.ticket_id ';
        $q .= ' INNER JOIN '.THREAD_ENTRY_TABLE.' e ';
        $q .= ' ON e.thread_id = r.id ';
        $q .= ' INNER JOIN '.USER_EMAIL_TABLE.' u ';
        $q .= ' ON u.user_id = t.user_id ';
        $q .= ' INNER JOIN '.TICKET_STATUS_TABLE.' s ';
        $q .= ' ON s.id = t.status_id ';
        // $q .= ' INNER JOIN '.TICKET_PRIORITY_TABLE.' p ';
        // $q .= ' ON p.id = r.status_id ';
        $q .= " ORDER BY id DESC ";
        $q .= " LIMIT ${range_len} OFFSET ${offset} ";
        // var_dump($q);
        // exit;
        $res = db_query($q);
